...
class ABIAllocator:
    ...
    def nextStackSlot(self):
        ...
        return f"process_stack_read({index})"
    ...
    def nextRegister(self, type):
        if type.isFloat():
            if self.float_reg_index == 8:
                ...
                return f"process_reg_read_f32(Register::s{index})"
        else:
            if self.reg_index == 4:
                return self.nextStackSlot()
            else:
                ...
                return f"process_reg_read_u32(Register::r{index})"
#-----------------------------------------------------------------------
...
def parse_func(line):
    ...
    if name in ignored_symbols:
        return ""
    code += f"static void {name}_trampoline() {{\n"
    ...
    if len(args):
        for i, arg in enumerate(args):
            ...
            if type.isPointer():
                code += f"({type.getType()}) process_mem_target_to_host({srcReg})"
    ...
    if ret_type != "void":
        if TypeParser(ret_type).isPointer():
            code += "\tprocess_reg_write_u32(Register::r0, process_mem_host_to_target((void*) r));\n"
        elif TypeParser(ret_type).isFloat():
            code += "\tprocess_reg_write_f32(Register::s0, (float) r);\n"
        else:
            code += "\tprocess_reg_write_u32(Register::r0, (uint32_t) r);\n"
    else:
        code += "\tprocess_reg_write_u32(Register::r0, 0);\n"
...
with open(argv[1], "r") as inputFile:
    ...
    out += "struct { const char* name; void* ptr; } static const sym_table[] = {\n"
